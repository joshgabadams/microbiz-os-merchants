...
class AgentCashOutServiceTest extends TestCase
{
    ...
    {
        $context = $this->makeReadyAgentContext(500000, ['single_transaction_limit' => 20000]);
        $customerAccount = $this->makeActiveCustomerAccount(200000);
        $user = User::factory()->create();
...
echo "AG-08 Part 2 of 2 applied (cash-out service, tests). Next: php artisan test --filter=AgentCashInServiceTest && php artisan test --filter=AgentCashOutServiceTest"