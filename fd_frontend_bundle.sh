...
mkdir -p src/app/features/fixed-deposits
...
export class FixedDepositListComponent implements OnInit {
  ...
  onBook(): void {
    const f = this.form;
    if (!f.customer_account_id || !f.settlement_account_id || !f.principal_amount || f.interest_rate === null || f.pre_liquidation_rate === null || !f.tenor_days) {
      return;
    }
    ...
    this.api
      ...
      .subscribe({
        next: () => {
          ...
          this.form = {
            customer_account_id: null, settlement_account_id: null, principal_amount: null,
            interest_rate: null, pre_liquidation_rate: null, pre_liquidation_penalty_fee: 0,
            tenor_days: null, narration: '',
          };
          this.load();
        },
        error: (err) => {
  ...
  confirmLiquidate(fd: FixedDeposit): void {
    ...
    this.api.liquidate(fd.id).subscribe({
      next: () => {
        this.busyId.set(null);
        this.previewingId.set(null);
        this.preview.set(null);
...
echo "Fixed Deposit frontend applied."
