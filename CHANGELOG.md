...
## Fixed (2026-08-16) — the Basic Auth switch (2026-08-14) had silently broken test authentication in 7 files, plus a fragile guard-resolution bug it exposed

**In plain terms:** Switching the API from Sanctum to stateless HTTP Basic Auth removed the `sanctum` guard from the request path entirely, but 7 existing test files still authenticated their simulated requests with `Sanctum::actingAs($user)` or `->actingAs($user, 'sanctum')` — calls that bind a user to the now-unused `sanctum` guard and have zero effect on a request that actually goes through `AuthenticateBasicOnce`. This went undetected because this environment has no local MySQL and the suite is hard-configured to require it, so these tests were never actually run after the Basic Auth switch landed. Running them today (via a temporary SQLite override, `DB_CONNECTION=sqlite DB_DATABASE=:memory: php artisan test`) surfaced it immediately: every one of those tests would have failed with 401s in any real MySQL-backed run (CI or otherwise).

**A second, more interesting bug fell out of fixing the first one:** one specific test (`AgentTransactionApiTest::test_cash_in_reversal_completes_full_http_maker_checker_flow`) called `->actingAs($checker, 'sanctum')`, which — via Laravel's own `Auth::shouldUse()` — changes the *default* guard for the rest of that test, not just the `sanctum` guard specifically. `AuthenticateBasicOnce` was calling the bare `Auth::onceBasic()` (no guard specified), so on this one request it resolved against the hijacked default (`sanctum`, a `RequestGuard`) instead of `web` (a `SessionGuard`), and `RequestGuard` has no `onceBasic()` method at all — a fatal `BadMethodCallException`, not just a 401. This was a real fragility in the middleware itself, not just a test artifact: relying on the Auth facade's mutable default guard instead of pinning one explicitly.

**Where:** `app/Http/Middleware/AuthenticateBasicOnce.php` (now `Auth::guard('web')->onceBasic()`, was bare `Auth::onceBasic()`). Test files: added `TestCase::actingAsBasicAuth()` (`tests/TestCase.php`) as the replacement helper, applied across `AgentAgreementApiTest`, `AgentTransactionApiTest`, `PermissionMiddlewareTest`, `AgentLocationApiTest`, `AgentComplaintApiTest`, `AgentActivationApiTest`, `AgentDashboardControllerTest`.

**Verified:** full suite run (SQLite override, since this environment still has no real MySQL) — **355 tests, 943 assertions, all passing**, including the previously-fatal maker-checker reversal test. Someone with a MySQL-backed environment should still re-run the real suite before this ships, per `phpunit.xml`'s own configuration, but this is no longer an untested guess.

## Architecture change (2026-08-14) — API auth switched from Sanctum tokens to stateless HTTP Basic Auth

**Why:** Requested directly -- Basic Auth is the standard most of the legacy third-party services this app will eventually integrate expect, and it already matches how `FineractClient` authenticates outbound to FinCore (`Http::withBasicAuth(...)`), so this makes the whole app consistent on one auth style instead of two.

**What changed:** `routes/api.php`'s single `auth:sanctum` group is now `auth.basic.once`, a new stateless middleware (`app/Http/Middleware/AuthenticateBasicOnce.php`, registered in `bootstrap/app.php`) built on `Auth::onceBasic()` -- validates the `Authorization: Basic` header against the `users` table on every request, no session or token persisted. `AuthController::login()`/`logout()` are gone (nothing left for them to do: no token to issue, nothing server-side to revoke); `AuthController::me()` is now the de facto login-verification endpoint. Frontend (`auth.service.ts`, `auth.interceptor.ts`): "login" is now `GET /v1/me` with a manually-attached Basic Auth header, storing the base64 credential in `localStorage` on success instead of a Sanctum bearer token; every subsequent request resends that same header via the interceptor; "logout" just discards it locally, no server round-trip.

**Two real, deliberate tradeoffs -- flagging rather than burying:**
1. **MFA enforcement is now fully gone**, not just harder to use. It was only ever checked inside the old `login()` (via `MfaService::verifyForUser()`), and Basic Auth's `email:password` header has no field to carry a one-time code on a per-request basis. `MfaController`/`MfaService`/the `mfa_enabled` column are all left in place untouched, but nothing calls into them anymore. Zero users currently have `mfa_enabled` set, so nothing regresses *today*, but this closes the door on it working again without a different mechanism (e.g. a separate device-registration step, not per-request).
2. **Every request now resends the password** (base64-encoded, not encrypted -- this is only as safe as the HTTPS transport it rides on) instead of a short-lived, individually revocable bearer token. There is no way to force one browser session to log out from the server side anymore; "logout" is purely the client discarding what it has stored.

**Verified:** `php -l` clean on all changed files; `tsc --noEmit` and `ng build` both clean. Live-tested via curl against the real running dev server: valid credentials -> 200 with user object; missing credentials -> 401; wrong password -> 401; a real RBAC-protected business endpoint (`GET /v1/agents`) succeeds under the new auth, confirming permission checks downstream still resolve `$request->user()` correctly. Did not complete a full browser click-through of the login form -- the Chrome extension lost page access mid-verification (confirmed to be an extension/tooling issue, not the app: it failed identically on unrelated read-only calls) -- but did confirm an unauthenticated session correctly redirects to `/login` before that happened.
...
**Where it was:** `app/Services/Payments/AgentOperationGuard.php:73` (`checkAgreementActive()`, was `where('status', 'ACTIVE')`) and `app/Models/Agent.php:93` (the `activeAgreement()` relationship, same wrong literal, currently unused elsewhere but scaffolded for this exact purpose) versus `app/Services/Payments/AgentAgreementService.php:638` (`executeAgreement()`, sets `'status' => 'EXECUTED'`). Also visible on the frontend: `frontend/src/app/features/agents/agent-detail.component.ts` gates the agreement's expanded "Manage" panel on `@if (agreement.status === 'EXECUTED')`, so a real executed agreement — genuinely stored as `EXECUTED` — was showing a blank expanded panel too (confirmed visually before the fix: agent id 3's V1 agreement expanded to nothing). (Independently found and fixed the same day by a colleague on `main`, commit `522f4ce` — same root cause, same fix, discovered on both sides of the merge before either saw the other's work.)
...
## Resolved by someone else, then re-broken by the same ACTIVE/EXECUTED bug, now fixed (2026-08-13) — AgentActivationService::activate() was stale dead code (found 2026-08-11), the TERMINAL_PENDING gate below has since been properly built out, but its own agreement check used the wrong status literal

**In plain terms:** The original problem described here (`activate()` only accepting `status === APPROVED`, which agreement execution moves the agent past) is gone — someone rebuilt this method since 2026-08-11 into a real, multi-condition gate: agent must be `TERMINAL_PENDING`, have an agreement, completed+acknowledged training, a verified active location, an active operator, and an active terminal, all checked explicitly. Good fix. But its agreement check has the *exact* same `ACTIVE`-vs-`EXECUTED` string mismatch documented above in the `AgentOperationGuard` entry — an independent inline query in this file, not something my earlier fix touched, so a genuinely executed agreement would still have failed this specific check and made `activate()` unreachable all over again for a different reason.

**Where:** `app/Services/Payments/AgentActivationService.php:39-41` (`activate()`, was `$agent->agreements()->where('status', 'ACTIVE')->exists()`). Also: nothing in the frontend ever called the `activate` endpoint at all — `AgentApiService.activate()` existed as a stub but no button anywhere invoked it, so even a correctly-gated agent had no way to actually finish onboarding through the UI.

**Fix applied:** Changed the query to `where('status', 'EXECUTED')`, matching the same canonical value used everywhere else. Added an "Activate Agent" button to the `TERMINAL_PENDING` banner in `agent-detail.component.ts`, wired to the existing `activate()` service call. Also discovered while testing this and the new Transactions screen: `agents.transactions.cash-out` (and its `agents.transactions.*` siblings) didn't exist in the local permissions table at all despite being defined in `PaymentsRbacSeeder` — same "seeder updated, never re-run against this DB" gotcha noted elsewhere in this file. Re-ran `PaymentsRbacSeeder` and re-synced the `admin` role's permissions to pick them up. Not run against the real PHPUnit suite for the same MySQL-availability reason as the entry above.
